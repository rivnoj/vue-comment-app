...
public class ByteQueue 
{
    ...
    public long popU8B() {
        return (((long) (pop() & 0xff) << 56) | ((long) (pop() & 0xff) << 48) | ((long) (pop() & 0xff) << 40) | ((long) (pop() & 0xff) << 32) | ((long) (pop() & 0xff) << 24) | ((long) (pop() & 0xff) << 16) | ((long) (pop() & 0xff) << 8)
                | (pop() & 0xff));
    }
    ...
    public ByteQueue clone() 
    {
    	...
    	{
    		byte[] ab = new byte[this.queue.length];
        	System.arraycopy(this.queue, 0, ab, 0, this.queue.length);
            return new ByteQueue(ab);	
    	}
    	else
    	{
    		return null;
    ...
    public String toString() {
        ...
        String aux = Integer.toHexString(peek(0) & 0xff);
        ...
        sb.append("[0x").append(aux);
        
        for (int i = 1; i < size; i++)
        {
        	aux = Integer.toHexString(peek(i) & 0xff);
    ...
    public String toString(int lastIndex) {
        ...
        for (int i = 1; i < ((lastIndex <= size())?lastIndex:size()) ; i++)
        {
        	aux = Integer.toHexString(peek(i) & 0xff);
